...
public class AggregateLearningOutcomeTest {
    ...
    this.mvc.perform(post("/learningOutcomes").with(csrf()).content(json)
        ...
        .andExpect(
            jsonPath("$.requirements[0].value",
                is(learningOutcomeToPost.getRequirements().get(0).getValue())))
        .andExpect(
            jsonPath("$.abilities[0].value",
                is(learningOutcomeToPost.getAbilities().get(0).getValue())))
        .andExpect(
            jsonPath("$.abilities[1].value",
                is(learningOutcomeToPost.getAbilities().get(1).getValue())))
        .andExpect(jsonPath("$.purposes[0].value",
            is(learningOutcomeToPost.getPurposes().get(0).getValue())))
  ...
  public void putLearningOutcome() throws Exception {
    ...
    this.mvc.perform(put("/learningOutcomes/" + learningOutcomeToPut.getId().toString())
        ...
        .andExpect(
            jsonPath("$.requirements[0].value",
                is(learningOutcomeToPut.getRequirements().get(0).getValue())))
        .andExpect(
            jsonPath("$.abilities[0].value",
                is(learningOutcomeToPut.getAbilities().get(0).getValue())))
        .andExpect(
            jsonPath("$.abilities[1].value",
                is(learningOutcomeToPut.getAbilities().get(1).getValue())))
        .andExpect(jsonPath("$.purposes[0].value",
            is(learningOutcomeToPut.getPurposes().get(0).getValue())))
        .andExpect(jsonPath("$._links.self", notNullValue()));
    ...
    this.entityManager.flush(); // Will force data to be persisted and events to trigger
  ...
  public void patchLearningOutcome() throws Exception {
    ...
    this.mvc.perform(
        ...
        .andExpect(
            jsonPath("$.purposes[0].value", is(learningOutcome.getPurposes().get(0).getValue())))
        .andExpect(jsonPath("$._links.self", notNullValue()));
  ...
  public void getLearningOutcomeByUUID() throws Exception {
    ...
    this.mvc.perform(get(url).with(csrf())).andExpect(status().isOk())
        ...
        .andExpect(
            jsonPath("$.requirements[0].value",
                is(learningOutcome.getRequirements().get(0).getValue())))
        .andExpect(
            jsonPath("$.abilities[0].value", is(learningOutcome.getAbilities().get(0).getValue())))
        .andExpect(
            jsonPath("$.abilities[1].value", is(learningOutcome.getAbilities().get(1).getValue())))
  ...
  private LearningOutcome buildSampleLearningOutcome() {
    ...
    return new LearningOutcome(role, competence, Arrays.asList(requirement),
        Arrays.asList(ability0, ability1), Arrays.asList(purpose));
